...
{
    /**
     * Informazioni che un team chiede davvero prima di chiudere uno step.
     *
     ...
     *
     * Stesso motivo della factory degli step: una `count(n)->create()` costruisce
     * tutte le istanze prima di salvarne una, e il massimo letto dal database
    ...
    public function definition(): array
    {
        ...
        return [
            'step_definition_id' => StepDefinition::factory(),
            'label' => $entry['label'],
            'type' => $entry['type'],
    ...
    {
        return $this->afterMaking(function (FieldDefinition $field): void {
            if ($field->getAttribute('position') !== null) {
                return;
            }

            $key = (string) $field->step_definition_id;

            $stored = (int) FieldDefinition::query()
                ->where('step_definition_id', $key)
                ->max('position');
    ...
    /**
     * Campo facoltativo: la sua assenza non impedisce di chiudere lo step.
     */
