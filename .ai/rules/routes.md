...
Tutte le altre rotte di questo progetto hanno doppia protezione (middleware `->can()` + Gate nel componente). La rotta di chiusura step e l'unica eccezione, ed e voluta: il middleware rifiuterebbe prima che il codice applicativo possa registrare il tentativo non autorizzato nel log e nel registro delle transizioni, che e un criterio di accettazione di US-005 (FR-012).
